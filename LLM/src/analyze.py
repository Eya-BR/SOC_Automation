"""
Clean SOC Alert Analysis System
No duplicates, no hallucinations, no errors
"""
...
class CleanAnalyzer:
    """Clean SOC-grade alert analyzer - no duplicates"""
    ...
    def __init__(self):
        ...
        logger.info(f"Clean Analyzer initialized - VT: {self.virustotal_available}")
    
    def analyze_alert(self, alert_data: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze alert with clean output - no duplicates"""
        try:
            ...
            # Clean LLM analysis
            llama_analysis = self._analyze_with_llama(alert_data)
            
            # Clean severity calculation
            final_severity = self._calculate_severity(alert_data, llama_analysis)
            threat_score = self._calculate_threat_score(final_severity, llama_analysis.get("confidence", 0.0))
            
            # Clean recommendations
            recommendations = self._generate_recommendations(alert_data, llama_analysis)
            
            # CLEAN OUTPUT STRUCTURE - NO DUPLICATES
            return {
                "alert_id": alert_id,
    ...
    def _analyze_with_llama(self, alert_data: Dict[str, Any]) -> Dict[str, Any]:
        """Clean LLM analysis - no hallucinations"""
        try:
            user = alert_data.get("result", {}).get("user", "")
            host = alert_data.get("result", {}).get("host", "")
            privilege = alert_data.get("result", {}).get("Privileges", "")
            
            # Get RAG MITRE techniques
            rag_mitre = self.mitre_loader.search_mitre_techniques(alert_data)
            
            # Machine account logic
            if user.endswith("$"):
                hypothesis = f"SeSecurityPrivilege usage detected on host {host}"
                confidence = 0.2
                legitimate_explanations = ["Machine account performing legitimate operations"]
                suspicious_indicators = []
                requires_investigation = False
            else:
                hypothesis = "Privilege usage detected - requires investigation"
                confidence = 0.4
    ...
    def _analyze_virustotal(self, observables: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        # Check IPs
        for ip in observables.get('ips', [])[:5]:  # Limit to 5 IPs
            result = self._check_virustotal_ip(ip)
            if result:
                vt_results.append(result)
        
        # Check domains
        for domain in observables.get('domains', [])[:5]:  # Limit to 5 domains
            result = self._check_virustotal_domain(domain)
            if result:
                vt_results.append(result)
        
        # Check hashes
        for hash_val in observables.get('hashes', [])[:5]:  # Limit to 5 hashes
    ...
    def _check_virustotal_ip(self, ip: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            params = {
                'apikey': self.tokens.get_token('virustotal'),
                'ip': ip
            }
